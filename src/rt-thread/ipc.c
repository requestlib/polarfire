...
rt_err_t rt_sem_take(rt_sem_t sem, rt_int32_t time)
{
    ...
    /* disable interrupt */
    // temp = rt_hw_interrupt_disable();
    temp = rt_spin_lock(&sem->parent.spin_lock);
    ...
    if (sem->value > 0)
    {
        ...
        /* enable interrupt */
        // rt_hw_interrupt_enable(temp);
        rt_spin_unlock(&sem->parent.spin_lock ,temp);
...
rt_err_t rt_mutex_take(rt_mutex_t mutex, rt_int32_t time)
{
    ...
    /* disable interrupt */
    // temp = rt_hw_interrupt_disable();
    temp = rt_spin_lock(&mutex->parent.spin_lock);
    ...
    if (mutex->owner == thread)
    {
        ...
        {
            // rt_hw_interrupt_enable(temp); /* enable interrupt */
            rt_spin_unlock(&mutex->parent.spin_lock ,temp);
...
rt_err_t rt_event_send(rt_event_t event, rt_uint32_t set)
{
    ...
    /* disable interrupt */
    // temp = rt_hw_interrupt_disable();
    temp = rt_spin_lock(&event->parent.spin_lock);
    ...
    if (!rt_list_isempty(&event->parent.suspend_thread))
    {
        ...
        while (n != &(event->parent.suspend_thread))
        {
            ...
            else
            {
                /* enable interrupt */
                // rt_hw_interrupt_enable(temp);
                rt_spin_unlock(&event->parent.spin_lock ,temp);
                         ...
                         rt_int32_t   timeout)
{
    ...
    /* disable interrupt */
    // temp = rt_hw_interrupt_disable();
    temp = rt_spin_lock(&mb->parent.spin_lock);
    ...
    {
        // rt_hw_interrupt_enable(temp);
        rt_spin_unlock(&mb->parent.spin_lock ,temp);
                         ...
                         rt_int32_t  timeout)
{
    ...
    /* disable interrupt */
    // temp = rt_hw_interrupt_disable();
    temp = rt_spin_lock(&mq->parent.spin_lock);
    ...
    if (msg == RT_NULL && timeout == 0)
    {
        /* enable interrupt */
        // rt_hw_interrupt_enable(temp);
        rt_spin_unlock(&mq->parent.spin_lock ,temp);
